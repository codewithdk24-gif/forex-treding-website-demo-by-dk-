...
export default function AuthPage() {
  ...
  // Note: Manual redirect (useEffect) removed. 
  // AuthContext handles all redirects automatically based on auth state.
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      console.log("[AuthPage] Starting Auth Flow:", { mode: authMode, email: cleanEmail });
      
      // Simple Regex Validation
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailRegex.test(cleanEmail)) {
        throw new Error("Invalid email format detected locally");
      }

      if (isAdminLogin) {
        // Admin Logic (Static for demo)
        if (cleanEmail === 'admin' && cleanPassword === '123456') {
          showNotification({ type: 'SUCCESS', message: 'Admin session authorized' });
          router.push('/admin'); // Admin redirect stays as it's separate from standard auth
        } else {
        ...
        if (data?.session) {
          showNotification({ type: 'SUCCESS', message: 'Account created and terminal connected' });
          // NO router.push here. AuthContext detects the new session and redirects.
        } else {
        ...
        // NO router.push here. AuthContext detects the login and redirects.
      }
    } catch (err) {
