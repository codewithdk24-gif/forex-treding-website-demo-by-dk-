...
const Icons = {
  lightning: (
    <svg className="icon-glow" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/>
    </svg>
  ),
  shield: (
    <svg className="icon-glow" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  check: (
    <svg className="text-green-500" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div className="min-h-screen bg-[#0f1115] text-white transition-all duration-300 overflow-x-hidden selection:bg-blue-500/20">
      ...
      <nav className="fixed top-0 w-full z-50 bg-[#0f1115]/80 backdrop-blur-md border-b border-white/[0.03] h-16 md:h-20 flex items-center justify-between px-6 lg:px-20">
        ...
        {/* Desktop nav links — MARKETS / EXECUTION / PRICING */}
        ...
        <div className="flex items-center gap-4">
          {/* JOIN NOW */}
          <Link href="/auth" className="btn-primary py-1.5 px-4 text-xs font-black uppercase tracking-widest rounded-lg h-8 md:h-10 w-auto flex items-center justify-center">
            Join Now
          </Link>
          <button onClick={() => toggleMenu(true)} className="lg:hidden p-2 text-gray-400 hover:text-white transition-colors">
            {Icons.menu}
          </button>
        </div>
      </nav>
      ...
      <div className={`fixed inset-0 z-[100] bg-[#0f1115]/95 backdrop-blur-xl transform ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'} transition-transform duration-500 p-8 flex flex-col justify-between`}>
        <div>
          <div className="flex justify-between items-center mb-12">
            ...
            <button onClick={() => toggleMenu(false)} className="p-2 text-gray-500 hover:text-white">{Icons.close}</button>
          </div>
          <div className="flex flex-col gap-6 text-xl font-black tracking-tighter uppercase">
            {[
              { label: 'Markets',       href: '#markets-page' },
              { label: 'Execution',     href: '#orders-page' },
              { label: 'Infrastructure',href: '#pricing-section' },
            ].map(({ label, href }) => (
              <a key={label} href={href} onClick={() => toggleMenu(false)} className="p-4 bg-white/5 rounded-2xl flex items-center justify-between group">
                <span>{label}</span>
        ...
        <div className="space-y-4">
          <Link href="/auth" onClick={() => toggleMenu(false)} className="btn-primary w-full py-4 text-center text-xs font-black uppercase tracking-widest rounded-2xl inline-block">
            Join Now
          </Link>
      ...
      {/* ── Hero Section ── */}
      <section className="hero-section relative flex items-center justify-center pt-32 pb-12 px-6 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto w-full flex flex-col lg:flex-row items-center gap-12 md:gap-20 relative z-10 reveal">
          ...
          <div className="flex-1 text-center lg:text-left space-y-8 md:space-y-12">
            <div className="flex flex-wrap justify-center lg:justify-start items-center gap-3 md:gap-6 opacity-60">
              {['0.01% Slippage', '0.4ms Execution', '99.99% Uptime'].map(stat => (
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-3">
              <Link href="/auth"      className="btn-primary w-full sm:w-auto px-8 py-3.5 text-xs font-black uppercase tracking-widest rounded-xl">Join Now</Link>
              <Link href="/dashboard" className="w-full sm:w-auto px-8 py-3.5 text-xs font-black text-gray-500 hover:text-white uppercase tracking-widest rounded-xl border border-white/5 hover:bg-white/5 transition-all">Explore Demo</Link>
            </div>

            {/* Mobile mini EUR/USD card */}
            <div className="lg:hidden w-full max-w-sm mx-auto mt-12 pt-4 reveal" style={{ transitionDelay: '0.3s' }}>
              <div className="glass-card p-6 rounded-[2rem] border border-white/5 shadow-2xl shadow-blue-600/5 relative overflow-hidden">
                <div className="flex justify-between items-center mb-6">
                  <div className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
                    <p className="text-xs font-black text-white uppercase tracking-widest">EUR / USD</p>
                  </div>
                  <span className="text-[10px] font-black text-blue-500 uppercase tracking-widest bg-blue-500/10 px-2 py-1 rounded-lg">Live</span>
                </div>
                <div className="flex justify-between items-end mb-6">
                  <div>
                    <p className="text-xs font-black text-gray-500 uppercase tracking-widest mb-1">EUR / USD</p>
                    <p className={`text-5xl font-black tracking-tighter transition-colors duration-300 ${mounted ? priceData.color : 'text-white'}`}>
          ...
          {/* Right — EUR/USD card (desktop) */}
          <div className="hidden lg:flex flex-1 items-center justify-center reveal" style={{ transitionDelay: '0.2s' }}>
            <div className="w-full max-w-md glass-card p-8 rounded-[2rem] border border-white/5 shadow-2xl shadow-blue-600/5 relative overflow-hidden group">
              ...
              <div className="flex justify-between items-center mb-6">
                ...
                <span className="text-[10px] font-black text-blue-500 uppercase tracking-widest bg-blue-500/10 px-2 py-1 rounded-lg">Live Feed</span>
              </div>

              {/* Live price */}
              <div className="mb-6">
                <p className="text-xs font-black text-gray-500 uppercase tracking-widest mb-2">Bid Price</p>
                <p className={`text-6xl font-black tracking-tighter transition-colors duration-300 ${mounted ? priceData.color : 'text-white'}`}>
                  {mounted ? priceData.bid : '1.08240'}
                </p>
                <div className="flex items-center gap-4 mt-2">
                  <p className="text-xs font-black text-gray-400 uppercase tracking-widest">Ask: {mounted ? priceData.ask : '1.08252'}</p>
                  <p className="text-xs font-black text-green-500 uppercase tracking-widest">+1.42%</p>
                </div>
              </div>

              {/* Mini chart */}
              <div className="h-28 w-full bg-blue-500/5 rounded-2xl border border-blue-500/10 relative overflow-hidden mb-6">
                <svg className="w-full h-full px-4" viewBox="0 0 400 100" preserveAspectRatio="none">
                  <path d="M0,80 Q50,70 100,85 T200,60 T300,75 T400,50" stroke="#3b82f6" strokeWidth="3" fill="none"/>
                  <path d="M0,80 Q50,70 100,85 T200,60 T300,75 T400,50 L400,100 L0,100 Z" fill="url(#blueGrad)" opacity="0.2"/>
                  <defs>
              ...
              <div className="flex justify-between items-center">
                <div className="text-center">
                  <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Spread</p>
                  <p className="text-sm font-black text-white">0.2 Pips</p>
                </div>
                <div className="text-center">
                  <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Execution</p>
                  <p className="text-sm font-black text-white">0.4ms</p>
                </div>
                <div className="text-center">
                  <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Leverage</p>
      ...
      {/* ── Institutional Market Feed (mobile) ── */}
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-between items-center px-4">
            <div className="space-y-1">
              <p className="text-xs font-black text-white uppercase tracking-widest">EUR / USD</p>
              <p className={`text-xs font-black animate-pulse ${mounted ? priceData.color : 'text-green-500'}`}>{mounted ? priceData.bid : '1.08240'}</p>
            </div>
      ...
      {/* ── Stats Section (MARKETS anchor) ── */}
      <section id="markets-page" className="max-w-7xl mx-auto px-6 py-12 md:py-16 reveal">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center bg-[#0f1115]/40 rounded-[1.9rem] divide-y md:divide-y-0 md:divide-x divide-white/[0.03]">
            {[
              ...
              <div key={i} className="w-full md:px-10 py-8 md:py-10 text-center space-y-1.5 hover:bg-white/[0.01] transition-colors group">
                <p className="text-xs font-bold text-blue-500/80 uppercase tracking-[0.2em] group-hover:-translate-y-0.5 transition-transform">{stat.label}</p>
      ...
      {/* ── Trust / Execution Section (EXECUTION anchor) ── */}
      <section id="orders-page" className="max-w-7xl mx-auto px-6 py-12 reveal">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            <div key={i} className="glass-card p-5 rounded-2xl border border-white/[0.03] flex flex-col items-center text-center space-y-2">
              <div className="w-10 h-10 bg-blue-600/5 rounded-xl flex items-center justify-center text-blue-500 transition-transform">
                {item.icon}
              </div>
      ...
      {/* ── Pricing / FAQ Section ── */}
      <section id="pricing-section" className="section-container py-24 px-6 max-w-3xl mx-auto space-y-12 reveal">
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={i} className={`glass-card rounded-2xl overflow-hidden border ${openFaqId === i ? 'border-blue-500/30' : 'border-white/[0.03]'} transition-all duration-300`}>
              <button
                id={`faq-btn-${i}`}
                onClick={() => setOpenFaqId(openFaqId === i ? -1 : i)}
                className="w-full min-h-[48px] p-6 text-left flex justify-between items-center hover:bg-white/[0.01] transition-colors focus:outline-none"
              >
                <span className="text-xs font-black text-white uppercase tracking-widest">{faq.q}</span>
                <span className={`transition-transform duration-300 text-blue-500 text-xs ${openFaqId === i ? 'rotate-180' : ''}`}>▼</span>
              </button>
              {openFaqId === i && (
                <div className="px-6 pb-6 text-xs text-gray-400 leading-relaxed font-medium">{faq.a}</div>
              )}
            </div>
          ))}
        </div>
