      ...
      <div class="flex items-center gap-3 md:gap-4">
        ...
        ${user.role === 'admin' ? `
          <div class="hidden sm:flex items-center gap-2 px-3 py-1 bg-blue-600/10 border border-blue-600/20 rounded-full">
            ...
            <span class="text-xs font-black text-blue-500 uppercase tracking-widest">Control Panel</span>
          </div>
        ` : ''}
      </div>
      ...
      <div class="flex items-center gap-4 md:gap-6">
        ...
        <div class="flex items-center gap-3">
          <div class="flex flex-col items-end hidden sm:flex">
            <p class="text-xs font-black text-white leading-none truncate max-w-[100px] uppercase">${user.name || 'Trader'}</p>
            <p class="text-xs font-black text-blue-500 uppercase tracking-widest mt-1">${user.role || 'User'}</p>
          </div>
