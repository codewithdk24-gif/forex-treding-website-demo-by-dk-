...
export default function Sidebar() {
  ...
  const adminItems = [
    ...
    { id: 'admin/transactions', icon: Icons.wallet, label: 'Transactions', path: '/admin/transactions' },
      ...
      <nav className="flex-1 px-3 xl:px-4 space-y-1">
        {menuItems.map(item => (
          <div key={item.id}
             ...
             ${pathname === item.path ? 'bg-blue-600/10 text-blue-500 shadow-sm' : 'text-gray-400 hover:bg-white/[0.03] hover:text-white'}`}>
      ...
      <div className="mt-auto space-y-4 pt-6 border-t border-white/5 pb-8 px-4">
        <div className="flex items-center justify-center xl:justify-start gap-4 px-2">
          ...
          <div className={`${isMobile ? 'block' : 'hidden xl:block'} min-w-0`}>
            <p className="text-sm font-black text-white truncate">{user.name || (isAdmin ? 'Platform Admin' : 'Trader')}</p>
            <p className="text-xs font-black text-blue-500 uppercase tracking-[0.2em] truncate">{user.role || 'User'}</p>
          </div>
        </div>
